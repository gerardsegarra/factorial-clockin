...
def login_and_get_session(email, password):
    ...
    if post_response.ok:
        ...
        print(f"Login failed with status code: {post_response.status_code}, error {post_response.reason}")
        return None
...
def get_datetimes(date_str):
    ...
    clock_out_afternoon_str = clock_out_afternoon.strftime("%Y-%m-%dT%H:%M:%S") + "+01:00"
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Create attendance shifts for Factorial.")
    ...
    first_clock_in, first_clock_out, second_clock_in, second_clock_out = get_datetimes(args.date)

    # Create attendance shifts using the authenticated session.
    create_attendance_shift(session, args.date, first_clock_in, first_clock_out, args.employee_id)
    create_attendance_shift(session, args.date, second_clock_in, second_clock_out, args.employee_id)
